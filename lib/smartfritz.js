    ...
    try {
        ...
        http.request({host:options.url, path:'/login_sid.lua'}, function(response) {
            ...
            response.on('end', function () {
                ...
                http.request({host:options.url, path:"/login_sid.lua?username=" + username + "&response="+challenge+"-" + require('crypto').createHash('md5').update(Buffer.from(challenge+'-'+password, 'UTF-16LE')).digest('hex')},function(response){
                    var str = '';
                    response.on('data', function (chunk) {
                        str += chunk;
                    });
...
// returns the value in 0.1C steps
module.exports.getTemperature = function(sid, ain, callback, options)
{
    options || (options = {});
    options.url || (options.url = 'fritz.box');

    http.request({host:options.url, path:'/webservices/homeautoswitch.lua?sid='+sid+'&switchcmd=gettemperature&ain='+ain},function(response){
        var data = '';
...
module.exports.getSwitchName = function(sid, ain, callback, options)
{
    options || (options = {});
    options.url || (options.url = 'fritz.box');

    http.request({host:options.url, path:'/webservices/homeautoswitch.lua?sid='+sid+'&switchcmd=getswitchname&ain='+ain},function(response){
        var data = '';
